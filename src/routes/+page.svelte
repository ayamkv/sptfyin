<script>
	...
	let pendingDeleteLink = $state(null);
	let deleteTurnstileVisible = $state(false);
	let deleteTurnstileReset = $state();
	let deleteTurnstileStatus = $state('pending');
	let currentItems = 4;
	...
	function resetDeleteTurnstile() {
		deleteTurnstileReset?.();
		deleteTurnstileStatus = 'pending';
		deleteTurnstileVisible = false;
	}

	function requestDeleteOwnedLink(link) {
		pendingDeleteLink = link;
		deleteTurnstileStatus = 'pending';
		deleteTurnstileVisible = true;
		deleteTurnstileReset?.();
	}

	async function deleteOwnedLink(link, turnstileToken) {
	...
	function handleDeleteTurnstileCallback(event) {
		deleteTurnstileStatus = 'verified';
		const token = event.detail.token;
		if (pendingDeleteLink) {
			deleteOwnedLink(pendingDeleteLink, token);
		}
	}
...
{#if useNewHome}
	<MobileHomeRedesign
		...
		{deletingOwnedLinkId}
		{pendingDeleteLink}
		{deleteTurnstileVisible}
		bind:deleteTurnstileReset
...
{:else}
	<div
		class="transform-y-2 relative flex h-[96vh] flex-col items-center justify-center overflow-auto rounded-2xl border-b-4 bg-background/50 pb-0 md:mt-0 md:rounded-xl md:border-2 md:border-b-0 md:border-r-0 md:pb-0 lg:min-h-[96vh] lg:overflow-hidden"
		...
		{:else}
			<div class="logo mt-[20em] flex flex-col items-center justify-center md:mb-6 md:mt-[2em]">
				...
				<div class="relative">
					...
					>
						<iconify-icon icon="lucide:link" class="mr-0.5 size-2.5 md:mr-1 md:size-4"
						></iconify-icon>
						...
						<Card.Content class="grid gap-4 pb-0 pt-6">
							<div>
								<form
									...
									class="flex w-[8rem] min-w-full flex-col"
								>
										...
										<Turnstile
											...
											on:callback={(event) => {
												...
												setTimeout(() => {
													if (
														turnstileStatus === 'verified' &&
														turnstileResponse === verifiedToken
													) {
														visible = false;
													}
												}, 250);
												...
												<div class="flex shrink-0 items-center gap-1 text-xs">
													{#if turnstileStatus === 'verified'}
														<iconify-icon
															...
															class="h-4 w-4 text-emerald-900"
														></iconify-icon>
						...
						<Card.Header>
										...
										<div class="buttons button-copy flex max-h-20 gap-1 lg:flex-col-reverse">
											{#each actions as action, i (action.icon)}
												...
												>
													...
													>
														<iconify-icon icon={action.icon} class="w-[24]" width="24">
														</iconify-icon>
													</Button>
												</div>
											{/each}
							...
							<div class="disclaim">
								...
								<p class="text-[10px] text-foreground/60">
									...
									this project <b>is NOT AFFILIATED</b> with, endorsed by, or sponsored by Spotify
									AB.
						...
						<div class="mt-2">
							...
							{:else if ownedLinks.length > 0}
								<div class="mb-4 rounded-xl border border-border/50 bg-card/40 p-3">
									...
									<p class="mb-3 text-xs text-muted-foreground/60">
										guests can delete their 3 newest local links after Turnstile verification.
									</p>
									{#if deleteTurnstileVisible && pendingDeleteLink}
										<div class="mb-3 rounded-xl border border-border/50 bg-background/40 p-3">
											<p class="mb-2 text-xs text-muted-foreground/70">
												verify to delete {pendingDeleteLink.id_url}
											</p>
											<Turnstile
												siteKey={turnstileKey}
												theme="dark"
												...
												on:error={() => {
													...
													toast.error('delete verification failed');
												}}
												on:expired={() => {
											...
											/>
											<div
												class="mt-2 flex items-center justify-between gap-3 text-xs text-muted-foreground/70"
											>
												<span
													>{deleteTurnstileStatus === 'verified'
														? 'deleting...'
										...
										{#each ownedLinks as item (item.id)}
											...
											>
												...
												<Button
													...
													on:click={() => requestDeleteOwnedLink(item)}
