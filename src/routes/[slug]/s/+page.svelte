...
>
	...
	<div class="logo mt-[2em] flex flex-col items-center justify-center">
		<h3
			class="md:text-md mt-4 flex justify-center rounded-md bg-orange-300 px-4 py-2 align-middle text-xs text-black"
		>
	...
	{#if visible}
		...
		>
				...
				<Card.Root class="relative w-[20rem] sm:w-[20rem] md:w-[25rem] lg:min-h-[16rem]">
					{#if !visible}
						<div in:fade out:fade={{ duration: 200 }}>
							<Skeleton class="absolute left-0 top-0 z-10 h-full w-full rounded-lg" />
						</div>
					{/if}
					<Card.Header>
			...
			<div class="right-card">
				<Card.Root
					class="relative mb-14 flex min-h-full w-[20rem] flex-col place-content-between transition-all sm:w-[20rem] md:mb-0 md:h-full lg:w-[35rem]"
				>
					{#if !visible}
						<div in:fade out:fade={{ duration: 200 }}>
							<Skeleton class="absolute left-0 top-0 z-10 h-full w-full rounded-lg" />
					...
					<Card.Header class="pb-2">
						...
						<Card.Content
							class="grid min-h-full overflow-y-auto px-0 pb-0 text-left text-muted-foreground/80 md:max-h-[26rem]"
						>
							<div
								class="align-center flex w-full min-w-full items-start justify-between py-2 transition-all md:py-2"
							>
								<Table.Root>
									<Table.Header>
										<Table.Row>
											<Table.Head class="border-r   border-white/10 pl-0 text-left md:w-[90px]"
												>country</Table.Head
											>
											<Table.Head class=" w-[140px] border-r border-white/10">browser</Table.Head>
											<Table.Head class=" border-r border-white/10">os</Table.Head>
										...
										{#if analytics && analytics.length}
											{#each getCurrentItems(analytics, currentPage, itemsPerPage) as v, i (i)}
												<Table.Row>
													...
													<Table.Cell class="border-r border-white/10">
													...
													<Table.Cell class="border-r border-white/10">
														<div class="flex items-center">
															<iconify-icon
																...
																class="mr-2 inline-block {getOs(v.utm_userAgent) === 'Android'
																	? 'text-[#3ddc84]'
																	: ''}"
																width="18"
																alt="OS icon"
					...
					<Card.Footer class="flex items-center justify-between pt-5">
						<Button variant="ghost2" onclick={previousPage} disabled={currentPage === 1}
							>prev</Button
						>
						<span class="text-sm text-muted-foreground">{currentPage} / {totalPages}</span>
						<Button variant="ghost2" onclick={nextPage} disabled={currentPage === totalPages}
							>next</Button
						>
					</Card.Footer>
				</Card.Root>
