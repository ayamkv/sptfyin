...
export async function GET({ locals, url, cookies }) {
	...
	if (!decodedVerifier) {
		...
		decodedVerifier = fallbackCookieVerifier
			? fallbackCookieVerifier.includes('%')
				? decodeURIComponent(fallbackCookieVerifier)
				: fallbackCookieVerifier
			: undefined;
	}
