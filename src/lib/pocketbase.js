...
export async function getRecentRecords(collection, perPage = 10, currentPage = 1) {
	const res = await fetch(
		`${pocketBaseURL}/api/collections/viewList/records?sort=-created&fields=id_url,from,created,subdomain&perPage=${perPage}&page=${currentPage}`
	);
	return res.json();
}
...
export async function getTotalClicks(collection, perPage = 10, currentPage = 1) {
	const res = await fetch(
		`${pocketBaseURL}/api/collections/analytics/records?sort=-created&perPage=${perPage}&page=${currentPage}`
	);
	return res.json();
}
